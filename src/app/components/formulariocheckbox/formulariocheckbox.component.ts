...
@Component({
  selector: 'app-formulariocheckbox',
  imports: [ReactiveFormsModule],
  templateUrl: './formulariocheckbox.component.html',
  styleUrl: './formulariocheckbox.component.css',
})
export class FormulariocheckboxComponent {
  conversion = '';
  potenciaNumero = '';
  cantidad = '';
  ...
  submit() {
    if (this.formularioConversion.value.numerodecimal) {
      if (this.formularioConversion.value.base == 'hexadecimal')
        this.conversion = parseInt(
          this.formularioConversion.value.numerodecimal
        ).toString(16);
      ...
      if (
        this.formularioConversion.value.numerodecimal &&
        this.formularioConversion.value.potencia
      )
      ...
      if (this.formularioConversion.value.largo)
        this.cantidad =
          this.formularioConversion.value.numerodecimal.length.toString();
    }
  }
}
